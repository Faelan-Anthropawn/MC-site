...
function makeItem() {
  return { id: '', count: 1, damage: 0, displayName: '', enchants: [], lockToInventory: false, collapsed: false };
}
...
function buildKitStructure(containerKey, items) {
  ...
  const nbtItems = items.map((item, i) => {
    ...
    if (item.lockToInventory)
      tag['minecraft:item_lock'] = 'lock_in_inventory';
  ...
  card.innerHTML = `
    ...
    <div class="kit-options-bar flex flex-col gap-3 p-3 rounded-lg bg-surface-900/50 border border-slate-700/30" style="display:${optionsDisplay}">
      ...
      <div class="flex items-center justify-between">
        ...
        <button type="button" class="kit-lock-toggle toggle-btn flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium rounded btn-secondary text-mist${item.lockToInventory ? ' active' : ''}" style="${item.lockToInventory ? '' : ''}">
