...
function makeItem() {
  return { id: '', count: 1, damage: 0, displayName: '', enchants: [], lockToInventory: false, keepOnDeath: false, collapsed: false };
}
...
function buildKitStructure(containerKey, items) {
  ...
  const nbtItems = items.map((item, i) => {
    ...
    if (item.lockToInventory)
      tag['minecraft:item_lock'] = nbtByte(1);
    if (item.keepOnDeath)
      tag['minecraft:keep_on_death'] = nbtByte(1);
...
function syncFromDOM() {
  ...
  list.querySelectorAll('.kit-item-card').forEach((card, i) => {
    ...
    kitItems[i].lockToInventory = card.querySelector('.kit-lock-toggle')?.classList.contains('active') ?? false;
    kitItems[i].keepOnDeath     = card.querySelector('.kit-death-toggle')?.classList.contains('active') ?? false;
...
function createItemCard(item, index) {
    ...
    <div class="kit-options-bar flex flex-col gap-3 p-3 rounded-lg bg-surface-900/50 border border-slate-700/30" style="display:${optionsDisplay}">
      ...
      <div class="flex items-center justify-between">
        <div class="flex flex-col gap-0.5">
          <span class="text-xs text-slate-400 font-medium">Lock to Inventory</span>
          <span class="text-xs text-slate-600">Prevents player from dropping or removing this item</span>
        </div>
        <button type="button" class="kit-lock-toggle toggle-btn flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium rounded btn-secondary text-mist${item.lockToInventory ? ' active' : ''}">
          <span class="material-symbols-outlined text-sm">lock${item.lockToInventory ? '' : '_open'}</span>
          <span class="kit-lock-label">${item.lockToInventory ? 'Locked' : 'Unlocked'}</span>
        </button>
      </div>

      <div class="flex items-center justify-between">
        <div class="flex flex-col gap-0.5">
          <span class="text-xs text-slate-400 font-medium">Keep on Death</span>
          <span class="text-xs text-slate-600">Item is kept in inventory when the player dies</span>
        </div>
        <button type="button" class="kit-death-toggle toggle-btn flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium rounded btn-secondary text-mist${item.keepOnDeath ? ' active' : ''}">
          <span class="material-symbols-outlined text-sm">${item.keepOnDeath ? 'shield' : 'shield_with_heart'}</span>
          <span class="kit-death-label">${item.keepOnDeath ? 'Kept' : 'Lost'}</span>
        </button>
      </div>
  ...
  const lockToggle = card.querySelector('.kit-lock-toggle');
  const lockIcon   = lockToggle.querySelector('.material-symbols-outlined');
  const lockLabel  = lockToggle.querySelector('.kit-lock-label');

  lockToggle.addEventListener('click', () => {
    kitItems[index].lockToInventory = !kitItems[index].lockToInventory;
    const locked = kitItems[index].lockToInventory;
  ...
  const deathToggle = card.querySelector('.kit-death-toggle');
  const deathIcon   = deathToggle.querySelector('.material-symbols-outlined');
  const deathLabel  = deathToggle.querySelector('.kit-death-label');

  deathToggle.addEventListener('click', () => {
    kitItems[index].keepOnDeath = !kitItems[index].keepOnDeath;
    const kept = kitItems[index].keepOnDeath;
    deathToggle.classList.toggle('active', kept);
    deathIcon.textContent  = kept ? 'shield' : 'shield_with_heart';
    deathLabel.textContent = kept ? 'Kept' : 'Lost';
  });
