...
async function addStructureVoidSupport(schem, baseGetKeyAt, onProgress) {
  ...
  if (blocks instanceof Uint32Array && paletteStr && paletteStr.length > 0) {
    ...
    for (let i = 0; i < volume; i++) {
      ...
      // Skip gravity blocks that were removed by hollowing
      if (baseGetKeyAt(i) === null) continue;
      ...
      if (y > 0) {
        ...
        // Treat below as air if it was hollowed away, or if it is already air in the palette
        if (airPal[blocks[belowIdx]] || baseGetKeyAt(belowIdx) === null) {
